...
X.Properties.VariableNames = {'buying','maint','doors','persons','lugboot','safety'};
Y2 = table(Ytrain);
X = [X Y2];
...
Y_Mdlc = Y_Mdl.';
Ytestc = Ytest.';
pod = sum(Ytestc(:) == 1); % this line is only used as a check-step for class counts, can be ignored.
figure(1);
...
b = TreeBagger(n,Xt,'Ytest','Method','classification','OOBVarImp','on',...
                          'CategoricalPredictors',06,...
                          'MinLeaf',leafs);
figure(7);
c = categorical({'buying','maint','doors','persons','lugboot','safety'});
